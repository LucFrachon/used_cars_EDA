---
title: "Used Cars Dataset - Exploratory Analysis"
author: "Luc Frachon"
...
require(plyr)
cars$vehicleType <- revalue(cars$vehicleType, c(andere = "other", bus = "people carrier", cabrio = "convertible",
                            coupe = "coupe", kleinwagen = "small car", kombi = "estate",
                            limousine = "sedan", suv = "SUV"))
cars$gearbox <- revalue(cars$gearbox, c(automatik = 'automatic', manuell = 'manual'))
cars$notRepairedDamage <- revalue(cars$notRepairedDamage, c(ja = 'yes', nein = 'no'))
cars$fuelType <- revalue(cars$fuelType, c(andere = 'other', benzin = 'petrol', cng = 'cng', 
                                          diesel = 'diesel', elektro = 'electric',
                                          hybrid = 'hybrid', lpg = 'lpg'))
cars$model <- revalue(cars$model, c(andere = 'other'))
cars$brand <- revalue(cars$brand, c(andere = 'other', sonstige_autos = 'other'))
...
I now look at the new variable summary statistics:

```{r}
summary(cars$price)
```

The mean has dropped by almost a factor 3 whereas the median has increased slightly, but I am much more confident in the resulting data.
...
ggcorr(cars[, .SD, .SDcols = c("yearOfRegistration", "powerPS", 
       ...
       low = 'darkred', mid = 'lightgrey', high = 'steelblue')
```

We notice fairly strong correlations between log(price) and power output, year of registration and mileage (negative in the latter case). Other correlations are weak.
We will build individual plots for the most promising variable pairs. Eventually I would like to be able to predict prices based on the other variables, so let's focus on price as one of our variables.


## 2.1. Continuous variables

### 2.1.1. Price vs. Power
...
### 2.1.2. Price vs. Power vs. Fuel Type
...
### 2.1.4. Price vs. Year of Registration
...
### 2.1.6. Year of Registration vs. Mileage
...
### 2.1.7. Ad Uptime vs. All Other Continuous Variables
...
### 2.2.1. Price and Power vs. Fuel Type

```{r fig.height=10, fig.width=8, fig.cap= "Brown diamonds indicate mean"}
...
So $p < .01$. We are very confident that there is a real difference in mean prices between the two groups.

### 2.2.2. Price vs. Gearbox
...
```{r fig.cap="Brown diamonds represent mean"}
...
### 2.2.6. Price vs. Unrepaired Damage
...
I decided to select the following variables:
 ...
 - **Discrete Predictors:**
    ...
    - `vehicleType`
    - `brand_cat`
    - `notRepairedDamage` in interaction with everything else. The reason for this is that we saw that this particular variable is associated with a 3- or 4-fold price difference, therefore for all intents and purposes it defines two separate markets (a bit like `collector_status`)
...
So this regression achieves $R^2=0.765$, which seems decent. There are several unsignificant parameters though, most of them for low-count fuel types such as electric or natural gas.
...
We see that some predictions are really close whereas others are widely off the mark. Let's make this more visual on a sample of 1,000 observations:
...
The mean is significantly higher than the median which confims the heavy right tail of the distribution. Indeed, the summary statistics show that the median price is €3,500 while the mean is at €6,317.

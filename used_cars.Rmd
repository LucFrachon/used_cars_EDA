...
Based on the knowledge that we gained on the previous phases of the project, I would like to attempt a linear regression to predict prices based on the most useful variables.
I decided to select the following ones:
...
```{r}
cars_no_nas <- na.omit(cars)
# train_indices <- sample(nrow(cars_no_nas), nrow(cars_no_nas) * .8)
# test_indices <- setdiff(seq(nrow(cars_no_nas)), train_indices)
# cars_train <- cars_no_nas[train_indices, ]
# cars_test <- cars_no_nas[test_indices, ]

fit <- lm(log_price ~ (powerPS + yearOfRegistration:collector_status + 
              kilometer:collector_status + gearbox:fuelType + vehicleType + 
